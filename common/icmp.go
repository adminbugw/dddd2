...
func CheckLive(hostslist []string, Ping bool) []string {
	ICMPStats.StartTime = time.Now()
	ICMPStats.TotalSent = int64(len(hostslist))

	gologger.AuditTimeLogger("ICMP发包探测存活，目标IP如下")
	gologger.AuditLogger(strings.Join(hostslist, ","))
	gologger.Info().Msgf("开始ICMP存活探测，目标数量: %d", len(hostslist))
	...
	go func() {
		...
		for {
			...
			case <-doneScan:
				return
			case <-ticker.C:
				elapsed := time.Since(ICMPStats.StartTime).Seconds()
				currentSuccess := int64(len(AliveHosts))
				progress := float64(currentSuccess) / float64(len(hostslist)) * 100

				gologger.Info().Msgf("ICMP探测进度: %d/%d (%.1f%%), 已用时间: %.1fs",
					currentSuccess, len(hostslist), progress, elapsed)
			}
		}
	}()
	...
	if Ping == true {
		...
		if err == nil {
			gologger.Info().Msg("使用ICMP原生套接字进行批量探测")
			RunIcmp1(hostslist, conn, chanHosts)
		} else {
			gologger.Info().Msg("尝试ICMP原生套接字失败，转为ICMP回显探测")
			...
			if err == nil {
				...
				gologger.Info().Msg("ICMP探测失败，转为Ping探测存活")
				RunPing(hostslist, chanHosts)
			}
		}
	}
	...
	ICMPStats.FailedCount = ICMPStats.TotalSent - ICMPStats.SuccessCount
	elapsed := time.Since(ICMPStats.StartTime).Seconds()
	avgSpeed := float64(len(hostslist)) / elapsed

	gologger.AuditTimeLogger("ICMP探测结束，发现存活主机: %d 个, 平均速度: %.2f hosts/s", len(AliveHosts), avgSpeed)
